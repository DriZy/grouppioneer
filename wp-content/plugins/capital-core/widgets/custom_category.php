...
class capital_custom_category extends WP_Widget {
	// constructor
	public function __construct() {
		 $widget_ops = array('description' => esc_html__( "Display latest and selected post categories of different post type.", 'capital-core') );
        parent::__construct(false, $name = esc_html__( 'Custom Categories','capital-core'), $widget_ops);
	}
	...
	?>
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('type')); ?>"><?php esc_html_e('Select Post Type', 'capital-core'); ?></label>
            <select class="spType" id="<?php echo esc_attr($this->get_field_id('type')); ?>" name="<?php echo esc_attr($this->get_field_name('type')); ?>">
                <?php
                $post_types = capital_get_all_types();
		    ...
		    if(!empty($post_types)){
                    foreach ( $post_types as $post_type ) {
                     ...
                     echo '<option value="no">'.esc_html__( 'No Post Type Found.','capital-core').'</option>';
                }
                ?>
            </select> 
        </p> 
...
// register widget
add_action('widgets_init', create_function('', 'return register_widget("capital_custom_category");'));
?>