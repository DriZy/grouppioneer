<?php
/*** Widget code for Flickr Images ***/
class capital_Flickr_Widget extends WP_Widget {

	var $defaults;

	public function __construct() {
		$widget_ops = array( 'classname' => 'capital_flickr_widget', 'description' => __( 'Display your Flickr photostream', 'capital-core' ) );
		$control_ops = array( 'id_base' => 'capital_flickr_widget' );
		parent::__construct( 'capital_flickr_widget', __( 'Flickr Widget', 'capital-core' ), $widget_ops, $control_ops );
		...
		$this->defaults = apply_filters( 'capital_flickr_widget_modify_defaults', $this->defaults );
	}
	...
	function get_photos( $id, $count = 8 ) {
		...
		$transient_key = md5( 'capital_flickr_cache_' . $id . $count );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title', 'capital-core' ); ?>:</label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'id' ); ?>"><?php _e( 'Flickr ID', 'capital-core' ); ?>:</label> <small><a href="http://idgettr.com/" target="_blank"><?php _e( 'What\'s my Flickr ID?', 'capital-core' ); ?></a></small>
			...
			<small class="howto"><?php _e( 'Example ID: 23100287@N07', 'capital-core' ); ?></small>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php _e( 'Number of photos', 'capital-core' ); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 't_width' ); ?>"><?php _e( 'Thumbnail width', 'capital-core' ); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 't_height' ); ?>"><?php _e( 'Thumbnail height', 'capital-core' ); ?>:</label>
			...
			<small class="howto"><?php _e( 'Note: You can use "0" value for auto height', 'capital-core' ); ?></small>
		</p>

		<p>
		<label for="<?php echo $this->get_field_id( 'randomize' ); ?>">
			<input type="checkbox" value="1" id="<?php echo $this->get_field_id( 'randomize' ); ?>" name="<?php echo $this->get_field_name( 'randomize' ); ?>" <?php checked( $instance['randomize'], 1 ); ?>/> <?php _e( 'Randomize photos?', 'capital-core' ); ?>
		</label>
		</p>
...
// register widget
add_action('widgets_init', create_function('', 'return register_widget("capital_flickr_widget");'));
?>
