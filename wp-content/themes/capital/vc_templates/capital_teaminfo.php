...
$address = get_post_meta(get_the_ID(), 'capital_staff_member_address', true);
$social = '';
$social_data = array();
...
if($tinfo_address && $address != ''){
	...
	$post_output .= '<div class="icon-box  ibox-outline ibox-icon-48 ibox-plain"><div class="ibox-icon accent-color"><i class="mi mi-location-on"></i></div><h3>'.esc_html('Postal address','capital').'</h3><p class="">'.$address.'</p></div>';
	$post_output .= '</div>';
}
if(($tinfo_email || $tinfo_phone) && ($email != '' || $phone != '')){
	$post_output .= '<div class="tinfo-block equal-height-column">';
	$post_output .= '<div class="icon-box  ibox-outline ibox-icon-48 ibox-plain"><div class="ibox-icon accent-color"><i class="mi mi-contacts"></i></div><h3>'.esc_html('Contact Info','capital').'</h3><p class="">';
	if($email != ''){
...
if($tinfo_social || $social != array()){
	$post_output .= '<div class="tinfo-block equal-height-column">';
	$post_output .= '<h3>'.esc_html('Social Profiles','capital').'</h3>';
	$post_output .= $social;
	$post_output .= '</div>';
}
$post_output .= '</div>';
